...
class COLISLY_Pricing {
	...
	public static function price_for_weight( $weight ) {
		...
		$tiers  = COLISLY_Settings::get( 'pricing_tiers', array() );
		$price  = null;
		...
		if ( null === $price ) {
			$base  = (float) COLISLY_Settings::get( 'price_base', 0 );
			$rate  = (float) COLISLY_Settings::get( 'price_per_kg', 0 );
		 ...
		 */
		return (float) apply_filters( 'colisly_parcel_price', round( $price, 2 ), $weight );
	}
}
