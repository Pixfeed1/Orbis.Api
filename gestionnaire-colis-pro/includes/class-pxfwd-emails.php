...
class PXFWD_Emails {
	...
	public static function init() {
		...
		add_action( 'pxfwd_parcel_created', array( __CLASS__, 'parcel_created' ), 10, 2 );
		add_action( 'pxfwd_shipment_requested', array( __CLASS__, 'shipment_requested' ), 10, 3 );
	}
	...
	public static function register_email_classes( $emails ) {
		require_once PXFWD_PLUGIN_DIR . 'includes/emails/class-pxfwd-email-parcel-received.php';
		require_once PXFWD_PLUGIN_DIR . 'includes/emails/class-pxfwd-email-shipment-requested.php';

		$emails['PXFWD_Email_Parcel_Received']    = new PXFWD_Email_Parcel_Received();
		$emails['PXFWD_Email_Shipment_Requested'] = new PXFWD_Email_Shipment_Requested();
	...
	public static function parcel_created( $parcel_id, $client ) {
		if ( ! PXFWD_Settings::get( 'notify_client_on_parcel', 1 ) ) {
			return;
		}
		...
		if ( self::wc_mailer() ) {
			 ...
			 * @param object $client    Client row.
			 */
			do_action( 'pxfwd_send_parcel_received_email', $parcel_id, $client );
	...
	public static function shipment_requested( $shipment_id, $client, $parcel_ids ) {
		if ( ! PXFWD_Settings::get( 'notify_admin_on_request', 1 ) ) {
			return;
		}
		...
		if ( self::wc_mailer() ) {
			 ...
			 * @param object $client      Client row.
			 */
			do_action( 'pxfwd_send_shipment_requested_email', $shipment_id, $client );
	...
	private static function shipment_requested_fallback( $shipment_id, $client, $parcel_ids ) {
		...
		$message = sprintf(
			...
			$client->reference,
			count( $parcel_ids ),
			PXFWD_Carriers::name( $shipment->carrier )
		);
