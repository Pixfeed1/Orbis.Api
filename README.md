...
Le plugin se trouve dans [`gestionnaire-colis-pro/`](gestionnaire-colis-pro/) et suit les standards du répertoire officiel WordPress.org (WordPress Coding Standards, i18n, nonces, échappement, tables préfixées créées via `dbDelta`).
...
- PHPCS : `cd gestionnaire-colis-pro && phpcs` (standards WordPress, ruleset `phpcs.xml.dist`)
