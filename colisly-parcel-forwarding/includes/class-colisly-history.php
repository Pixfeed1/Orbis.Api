...
class COLISLY_History {
	...
	public static function log( $client_id, $event, $message, $parcel_id = 0, $shipment_id = 0 ) {
		 ...
		 */
		do_action( 'colisly_history_logged', $client_id, $event, $message );
	}
	...
	public static function for_client( $client_id, $limit = 100 ) {
		...
		return $wpdb->get_results(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}colisly_history WHERE client_id = %d ORDER BY id DESC LIMIT %d",
				(int) $client_id,
				(int) $limit
			)
		);
