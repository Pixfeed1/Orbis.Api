...
class COLISLY_Admin_Parcels {
	...
	public static function render_list() {
		...
		<div class="wrap colisly-wrap">
			<h1 class="wp-heading-inline"><?php esc_html_e( 'Parcels', 'colisly-parcel-forwarding' ); ?></h1>
			<a href="<?php echo esc_url( admin_url( 'admin.php?page=colisly-new-parcel' ) ); ?>" class="page-title-action"><?php esc_html_e( 'New parcel', 'colisly-parcel-forwarding' ); ?></a>
			...
			<form method="get" class="colisly-search-form">
				...
				<p class="search-box">
					<label class="screen-reader-text" for="colisly-parcel-search"><?php esc_html_e( 'Search parcels', 'colisly-parcel-forwarding' ); ?></label>
					<input type="search" id="colisly-parcel-search" name="s" value="<?php echo esc_attr( $search ); ?>" placeholder="<?php esc_attr_e( 'Parcel number, tracking, client…', 'colisly-parcel-forwarding' ); ?>" />
					<label class="screen-reader-text" for="colisly-parcel-status"><?php esc_html_e( 'Status', 'colisly-parcel-forwarding' ); ?></label>
					<select name="status" id="colisly-parcel-status">
						<option value=""><?php esc_html_e( 'All statuses', 'colisly-parcel-forwarding' ); ?></option>
						<?php foreach ( COLISLY_Parcels::statuses() as $key => $label ) : ?>
							<option value="<?php echo esc_attr( $key ); ?>" <?php selected( $status, $key ); ?>><?php echo esc_html( $label ); ?></option>
						<?php endforeach; ?>
					</select>
					<button type="submit" class="button"><?php esc_html_e( 'Filter', 'colisly-parcel-forwarding' ); ?></button>
				</p>
			</form>
			...
			<table class="wp-list-table widefat fixed striped">
				<thead>
					<tr>
						...
						<th><?php esc_html_e( 'Client', 'colisly-parcel-forwarding' ); ?></th>
				...
				<tbody>
					<?php if ( empty( $result['items'] ) ) : ?>
						<tr><td colspan="8"><?php esc_html_e( 'No parcels found.', 'colisly-parcel-forwarding' ); ?></td></tr>
					<?php else : ?>
	...
	public static function render_new() {
		...
		?>
		<div class="wrap colisly-wrap">
			<h1><?php esc_html_e( 'New parcel', 'colisly-parcel-forwarding' ); ?></h1>
			<?php COLISLY_Admin::maybe_notice(); ?>
			...
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" enctype="multipart/form-data" id="colisly-new-parcel-form">
				...
				<h2><?php esc_html_e( '1. Client', 'colisly-parcel-forwarding' ); ?></h2>
				<table class="form-table" role="presentation">
					<tr>
						<th scope="row"><label for="colisly-client-search-input"><?php esc_html_e( 'Search for the client', 'colisly-parcel-forwarding' ); ?></label></th>
						<td>
							<input
								type="text"
								id="colisly-client-search-input"
								class="regular-text"
								placeholder="<?php esc_attr_e( 'Reference (CL000001), name or e-mail…', 'colisly-parcel-forwarding' ); ?>"
								autocomplete="off"
							...
							<p class="description"><?php esc_html_e( 'Search by internal reference, name or e-mail address.', 'colisly-parcel-forwarding' ); ?></p>
						</td>
					</tr>
				</table>
				...
				<div id="colisly-client-stock">
					<?php if ( $client ) : ?>
						<h3><?php esc_html_e( 'Parcels of this client still in stock', 'colisly-parcel-forwarding' ); ?></h3>
						...
						<table class="wp-list-table widefat fixed striped colisly-stock-table">
							...
							<tbody>
								<?php if ( empty( $stock ) ) : ?>
									<tr><td colspan="4"><?php esc_html_e( 'No parcels in stock for this client.', 'colisly-parcel-forwarding' ); ?></td></tr>
								<?php else : ?>
				...
				<h2><?php esc_html_e( '2. Parcel information', 'colisly-parcel-forwarding' ); ?></h2>
				<table class="form-table" role="presentation">
					<tr>
						<th scope="row"><label for="colisly-tracking"><?php esc_html_e( 'Carrier tracking number', 'colisly-parcel-forwarding' ); ?></label></th>
						<td><input type="text" id="colisly-tracking" name="tracking_number" class="regular-text" /></td>
					</tr>
					<tr>
						<th scope="row"><label for="colisly-weight"><?php esc_html_e( 'Real weight (kg)', 'colisly-parcel-forwarding' ); ?> <span class="description">(<?php esc_html_e( 'required', 'colisly-parcel-forwarding' ); ?>)</span></label></th>
						<td><input type="number" id="colisly-weight" name="weight" step="0.001" min="0.001" required /></td>
					</tr>
					<tr>
						<th scope="row"><?php esc_html_e( 'Dimensions (cm, visible to administrators only)', 'colisly-parcel-forwarding' ); ?></th>
						<td class="colisly-dimensions">
							<label for="colisly-length"><?php esc_html_e( 'Length', 'colisly-parcel-forwarding' ); ?></label>
							<input type="number" id="colisly-length" name="length" step="0.01" min="0" />
							<label for="colisly-width"><?php esc_html_e( 'Width', 'colisly-parcel-forwarding' ); ?></label>
							<input type="number" id="colisly-width" name="width" step="0.01" min="0" />
							<label for="colisly-height"><?php esc_html_e( 'Height', 'colisly-parcel-forwarding' ); ?></label>
							<input type="number" id="colisly-height" name="height" step="0.01" min="0" />
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="colisly-photo"><?php esc_html_e( 'Photo of the parcel at reception (optional)', 'colisly-parcel-forwarding' ); ?></label></th>
						<td><input type="file" id="colisly-photo" name="colisly_photo" accept="image/*" /></td>
					</tr>
					<tr>
						<th scope="row"><label for="colisly-note"><?php esc_html_e( 'Internal comment (never visible to the client)', 'colisly-parcel-forwarding' ); ?></label></th>
						<td><textarea id="colisly-note" name="internal_note" rows="3" class="large-text" placeholder="<?php esc_attr_e( 'Damaged packaging, fragile parcel, anomaly…', 'colisly-parcel-forwarding' ); ?>"></textarea></td>
					</tr>
				</table>

				<h2><?php esc_html_e( '3. Shipping rules', 'colisly-parcel-forwarding' ); ?></h2>
				<table class="form-table" role="presentation">
					<tr>
						<th scope="row"><?php esc_html_e( 'Grouping', 'colisly-parcel-forwarding' ); ?></th>
						<td>
							<label>
								<input type="checkbox" name="allow_grouping" value="1" checked />
								<?php esc_html_e( 'Allow this parcel to be grouped with other parcels', 'colisly-parcel-forwarding' ); ?>
							</label>
							<p class="description"><?php esc_html_e( 'If unchecked, this parcel will have to be shipped alone. This decision can never be changed by the client.', 'colisly-parcel-forwarding' ); ?></p>
						</td>
					</tr>
					<tr>
						<th scope="row"><?php esc_html_e( 'Allowed carriers', 'colisly-parcel-forwarding' ); ?></th>
						<td>
							...
							<?php endforeach; ?>
							<p class="description"><?php esc_html_e( 'Uncheck incompatible carriers (oversize, restrictions…). Only checked carriers will be offered to the client.', 'colisly-parcel-forwarding' ); ?></p>
						</td>
					</tr>
				</table>

				<p class="description">
					<?php esc_html_e( 'The parcel price is computed automatically from its weight when saved, then used for the shipment request.', 'colisly-parcel-forwarding' ); ?>
				</p>

				<?php submit_button( __( 'Save the parcel', 'colisly-parcel-forwarding' ) ); ?>
			</form>
		</div>
		<?php
	}
	...
	public static function handle_create() {
		...
		COLISLY_Admin::redirect(
			...
			sprintf(
				...
				__( 'Parcel %s saved. Its price has been computed automatically.', 'colisly-parcel-forwarding' ),
				$parcel->reference
			)
		);
	}
	...
	public static function handle_set_status() {
		...
		COLISLY_Admin::redirect( 'colisly-clients', array( 'client' => $client_id ), __( 'Parcel status updated.', 'colisly-parcel-forwarding' ) );
	}
}
