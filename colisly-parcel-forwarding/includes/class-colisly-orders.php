...
class COLISLY_Orders {
	...
	public static function create_for_shipment( $shipment, $client ) {
		...
		if ( ! self::available() ) {
			return new WP_Error( 'colisly_wc_missing', __( 'WooCommerce is not available.', 'colisly-parcel-forwarding' ) );
		}

		$order = wc_create_order(
			array(
				'customer_id' => (int) $client->user_id,
				'created_via' => 'colisly-parcel-forwarding',
			)
		);
		...
		foreach ( COLISLY_Shipments::parcels( (int) $shipment->id ) as $parcel ) {
			...
			$fee->set_name(
				sprintf(
					...
					__( 'Parcel %1$s (%2$s kg)', 'colisly-parcel-forwarding' ),
					$parcel->reference,
		...
		if ( (float) $shipment->storage_fees > 0 ) {
			$fee = new WC_Order_Item_Fee();
			$fee->set_name( __( 'Storage fees', 'colisly-parcel-forwarding' ) );
			$fee->set_tax_status( $tax_status );
		...
		$order->add_order_note(
			sprintf(
				...
				__( 'Order created for shipment request %s.', 'colisly-parcel-forwarding' ),
				$shipment->reference
			)
		);
		...
		COLISLY_History::log(
			...
			sprintf(
				...
				__( 'WooCommerce order #%1$s created for shipment %2$s.', 'colisly-parcel-forwarding' ),
	...
	public static function sync_from_shipment( $shipment, $status ) {
		...
		if ( 'shipped' === $status && ! $order->has_status( array( 'completed', 'cancelled', 'refunded' ) ) ) {
			$order->update_status( 'completed', sprintf( /* translators: %s: shipment reference. */ __( 'Shipment %s shipped.', 'colisly-parcel-forwarding' ), $shipment->reference ) );
		} elseif ( 'cancelled' === $status && $order->has_status( array( 'pending', 'on-hold', 'failed' ) ) ) {
			$order->update_status( 'cancelled', sprintf( /* translators: %s: shipment reference. */ __( 'Shipment %s cancelled.', 'colisly-parcel-forwarding' ), $shipment->reference ) );
		}
