...
class COLISLY_Shipments {
	...
	public static function statuses() {
		return array(
			'requested'        => _x( 'Requested', 'shipment status', 'colisly-parcel-forwarding' ),
			'awaiting_payment' => _x( 'Awaiting payment', 'shipment status', 'colisly-parcel-forwarding' ),
			'paid'             => _x( 'Paid', 'shipment status', 'colisly-parcel-forwarding' ),
			'preparing'        => _x( 'Preparing', 'shipment status', 'colisly-parcel-forwarding' ),
			'shipped'          => _x( 'Shipped', 'shipment status', 'colisly-parcel-forwarding' ),
			'cancelled'        => _x( 'Cancelled', 'shipment status', 'colisly-parcel-forwarding' ),
		);
	}
	...
	public static function request( $client_id, $parcel_ids, $carrier ) {
		...
		if ( empty( $parcel_ids ) ) {
			return new WP_Error( 'colisly_no_parcels', __( 'Select at least one parcel.', 'colisly-parcel-forwarding' ) );
		}

		$carrier = sanitize_key( $carrier );
		$enabled = wp_list_pluck( COLISLY_Carriers::all( true ), 'slug' );
		if ( ! in_array( $carrier, $enabled, true ) ) {
			return new WP_Error( 'colisly_invalid_carrier', __( 'Invalid carrier.', 'colisly-parcel-forwarding' ) );
		}
		...
		foreach ( $parcel_ids as $parcel_id ) {
			...
			if ( ! $parcel || (int) $parcel->client_id !== (int) $client->id ) {
				return new WP_Error( 'colisly_invalid_parcel', __( 'Parcel not found for this client.', 'colisly-parcel-forwarding' ) );
			}

			if ( 'available' !== $parcel->status ) {
				/* translators: %s: parcel reference. */
				return new WP_Error( 'colisly_parcel_unavailable', sprintf( __( 'Parcel %s is no longer available.', 'colisly-parcel-forwarding' ), $parcel->reference ) );
			}

			if ( ! $parcel->allow_grouping && count( $parcel_ids ) > 1 ) {
				/* translators: %s: parcel reference. */
				return new WP_Error( 'colisly_grouping_forbidden', sprintf( __( 'Parcel %s must be shipped alone (grouping forbidden).', 'colisly-parcel-forwarding' ), $parcel->reference ) );
			}

			$allowed = COLISLY_Parcels::allowed_carrier_slugs( $parcel );
			if ( ! empty( $allowed ) && ! in_array( $carrier, $allowed, true ) ) {
				/* translators: %s: parcel reference. */
				return new WP_Error( 'colisly_carrier_forbidden', sprintf( __( 'The chosen carrier is not allowed for parcel %s.', 'colisly-parcel-forwarding' ), $parcel->reference ) );
			}
		...
		if ( ! $inserted ) {
			return new WP_Error( 'colisly_db_error', __( 'The shipment request could not be created.', 'colisly-parcel-forwarding' ) );
		}
		...
		COLISLY_History::log(
			...
			sprintf(
				...
				__( 'Shipment request %1$s created (%2$d parcels, carrier %3$s).', 'colisly-parcel-forwarding' ),
				$reference,
				count( $parcel_ids ),
	...
	public static function set_status( $shipment_id, $status ) {
		...
		if ( ! array_key_exists( $status, self::statuses() ) ) {
			return new WP_Error( 'colisly_invalid_status', __( 'Invalid shipment status.', 'colisly-parcel-forwarding' ) );
		}

		$shipment = self::get( $shipment_id );
		if ( ! $shipment ) {
			return new WP_Error( 'colisly_invalid_shipment', __( 'Shipment not found.', 'colisly-parcel-forwarding' ) );
		}
		...
		COLISLY_History::log(
			...
			sprintf(
				...
				__( 'Shipment %1$s: status “%2$s” → “%3$s”.', 'colisly-parcel-forwarding' ),
				$shipment->reference,
