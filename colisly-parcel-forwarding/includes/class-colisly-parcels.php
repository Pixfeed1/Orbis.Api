...
class COLISLY_Parcels {
	...
	public static function statuses() {
		return array(
			'available'        => _x( 'Available', 'parcel status', 'colisly-parcel-forwarding' ),
			'ordered'          => _x( 'Ordered', 'parcel status', 'colisly-parcel-forwarding' ),
			'awaiting_payment' => _x( 'Awaiting payment', 'parcel status', 'colisly-parcel-forwarding' ),
			'paid'             => _x( 'Paid', 'parcel status', 'colisly-parcel-forwarding' ),
			'preparing'        => _x( 'Preparing', 'parcel status', 'colisly-parcel-forwarding' ),
			'shipped'          => _x( 'Shipped', 'parcel status', 'colisly-parcel-forwarding' ),
			'destroyed'        => _x( 'Destroyed', 'parcel status', 'colisly-parcel-forwarding' ),
			'cancelled'        => _x( 'Cancelled', 'parcel status', 'colisly-parcel-forwarding' ),
		);
	}
	...
	public static function create( $data ) {
		...
		if ( $weight <= 0 ) {
			return new WP_Error( 'colisly_invalid_weight', __( 'The parcel weight must be greater than zero.', 'colisly-parcel-forwarding' ) );
		}
		...
		if ( ! $inserted ) {
			return new WP_Error( 'colisly_db_error', __( 'The parcel could not be saved.', 'colisly-parcel-forwarding' ) );
		}
		...
		COLISLY_History::log(
			...
			sprintf(
				...
				__( 'Parcel %1$s received (%2$s kg, price %3$s).', 'colisly-parcel-forwarding' ),
				$reference,
	...
	public static function set_status( $parcel_id, $status ) {
		...
		if ( ! array_key_exists( $status, self::statuses() ) ) {
			return new WP_Error( 'colisly_invalid_status', __( 'Invalid parcel status.', 'colisly-parcel-forwarding' ) );
		}

		$parcel = self::get( $parcel_id );
		if ( ! $parcel ) {
			return new WP_Error( 'colisly_invalid_parcel', __( 'Parcel not found.', 'colisly-parcel-forwarding' ) );
		}
		...
		if ( false === $updated ) {
			return new WP_Error( 'colisly_db_error', __( 'The status could not be updated.', 'colisly-parcel-forwarding' ) );
		}
		...
		COLISLY_History::log(
			...
			sprintf(
				...
				__( 'Parcel %1$s: status “%2$s” → “%3$s”.', 'colisly-parcel-forwarding' ),
				$parcel->reference,
