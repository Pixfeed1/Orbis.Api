...
class COLISLY_Emails {
	...
	public static function init() {
		...
		add_action( 'colisly_parcel_created', array( __CLASS__, 'parcel_created' ), 10, 2 );
		add_action( 'colisly_shipment_requested', array( __CLASS__, 'shipment_requested' ), 10, 3 );
	}
	...
	public static function register_email_classes( $emails ) {
		require_once COLISLY_PLUGIN_DIR . 'includes/emails/class-colisly-email-parcel-received.php';
		require_once COLISLY_PLUGIN_DIR . 'includes/emails/class-colisly-email-shipment-requested.php';

		$emails['COLISLY_Email_Parcel_Received']    = new COLISLY_Email_Parcel_Received();
		$emails['COLISLY_Email_Shipment_Requested'] = new COLISLY_Email_Shipment_Requested();
	...
	public static function parcel_created( $parcel_id, $client ) {
		if ( ! COLISLY_Settings::get( 'notify_client_on_parcel', 1 ) ) {
			return;
		}
		...
		if ( self::wc_mailer() ) {
			 ...
			 * @param object $client    Client row.
			 */
			do_action( 'colisly_send_parcel_received_email', $parcel_id, $client );
	...
	public static function shipment_requested( $shipment_id, $client, $parcel_ids ) {
		if ( ! COLISLY_Settings::get( 'notify_admin_on_request', 1 ) ) {
			return;
		}
		...
		if ( self::wc_mailer() ) {
			 ...
			 * @param object $client      Client row.
			 */
			do_action( 'colisly_send_shipment_requested_email', $shipment_id, $client );
	...
	private static function parcel_created_fallback( $parcel_id, $client ) {
		...
		$subject = sprintf(
			...
			__( '[%1$s] Your parcel %2$s has been received', 'colisly-parcel-forwarding' ),
	...
	private static function shipment_requested_fallback( $shipment_id, $client, $parcel_ids ) {
		...
		$subject = sprintf(
			...
			__( 'New shipment request %1$s (client %2$s)', 'colisly-parcel-forwarding' ),
			$shipment->reference,
			$client->reference
		);
