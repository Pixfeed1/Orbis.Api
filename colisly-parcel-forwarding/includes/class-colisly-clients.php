...
class COLISLY_Clients {
	...
	public static function create( $user_id, $phone = '' ) {
		...
		if ( ! $user ) {
			return new WP_Error( 'colisly_invalid_user', __( 'User not found.', 'colisly-parcel-forwarding' ) );
		}
		...
		if ( ! $inserted ) {
			return new WP_Error( 'colisly_db_error', __( 'The client record could not be created.', 'colisly-parcel-forwarding' ) );
		}
		...
		COLISLY_History::log( $client_id, 'client_created', sprintf( /* translators: %s: client reference. */ __( 'Client record %s created.', 'colisly-parcel-forwarding' ), $reference ) );
