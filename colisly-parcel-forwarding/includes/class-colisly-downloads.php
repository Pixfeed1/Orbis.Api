...
class COLISLY_Downloads {
	...
	public static function init() {
		add_action( 'admin_post_colisly_download_document', array( __CLASS__, 'document' ) );
		add_action( 'admin_post_nopriv_colisly_download_document', array( __CLASS__, 'require_login' ) );
		add_action( 'admin_post_colisly_parcel_photo', array( __CLASS__, 'photo' ) );
		add_action( 'admin_post_nopriv_colisly_parcel_photo', array( __CLASS__, 'require_login' ) );
	}
	...
	public static function document_url( $document ) {
		return wp_nonce_url(
			add_query_arg(
				array(
					'action'   => 'colisly_download_document',
					'document' => (int) $document->id,
				),
				admin_url( 'admin-post.php' )
			),
			'colisly_download_document_' . (int) $document->id
		);
	}
	...
	public static function photo_url( $parcel ) {
		return wp_nonce_url(
			add_query_arg(
				array(
					'action' => 'colisly_parcel_photo',
					'parcel' => (int) $parcel->id,
				),
				admin_url( 'admin-post.php' )
			),
			'colisly_parcel_photo_' . (int) $parcel->id
		);
	}
	...
	public static function user_can_download_document( $document, $user_id ) {
		if ( user_can( $user_id, 'colisly_manage' ) ) {
			return true;
		}
		...
		$client = COLISLY_Clients::get( (int) $document->client_id );
	...
	public static function document() {
		...
		check_admin_referer( 'colisly_download_document_' . $document_id );

		$document = COLISLY_Documents::get( $document_id );

		if ( ! $document || empty( $document->file_path ) ) {
			wp_die( esc_html__( 'Document not found.', 'colisly-parcel-forwarding' ), '', array( 'response' => 404 ) );
		}

		if ( ! self::user_can_download_document( $document, get_current_user_id() ) ) {
			wp_die( esc_html__( 'Access denied.', 'colisly-parcel-forwarding' ), '', array( 'response' => 403 ) );
		}

		COLISLY_Files::send( $document->file_path, $document->file_name ? $document->file_name : $document->title );
	}
	...
	public static function photo() {
		...
		check_admin_referer( 'colisly_parcel_photo_' . $parcel_id );
		...
		if ( ! $parcel || empty( $parcel->photo_path ) ) {
			wp_die( esc_html__( 'Photo not found.', 'colisly-parcel-forwarding' ), '', array( 'response' => 404 ) );
		}

		COLISLY_Files::send( $parcel->photo_path, $parcel->reference . '-photo', true );
	}
}
