...
class COLISLY_Privacy {
	...
	public static function register_exporter( $exporters ) {
		$exporters['colisly-parcel-forwarding'] = array(
			'exporter_friendly_name' => __( 'Colisly Parcel Forwarding', 'colisly-parcel-forwarding' ),
	...
	public static function register_eraser( $erasers ) {
		$erasers['colisly-parcel-forwarding'] = array(
			'eraser_friendly_name' => __( 'Colisly Parcel Forwarding', 'colisly-parcel-forwarding' ),
	...
	public static function export( $email ) {
		...
		$items[] = array(
			...
			'group_label' => __( 'Client record (Colisly Parcel Forwarding)', 'colisly-parcel-forwarding' ),
			'item_id'     => 'colisly-client-' . (int) $client->id,
			'data'        => array(
				array(
					'name'  => __( 'Client reference', 'colisly-parcel-forwarding' ),
					'value' => $client->reference,
				),
				array(
					'name'  => __( 'Phone', 'colisly-parcel-forwarding' ),
					'value' => $client->phone,
				),
				array(
					'name'  => __( 'Created on', 'colisly-parcel-forwarding' ),
		...
		foreach ( COLISLY_Parcels::for_client( (int) $client->id ) as $parcel ) {
			$items[] = array(
				...
				'group_label' => __( 'Parcels (Colisly Parcel Forwarding)', 'colisly-parcel-forwarding' ),
				'item_id'     => 'colisly-parcel-' . (int) $parcel->id,
				'data'        => array(
					array(
						'name'  => __( 'Parcel number', 'colisly-parcel-forwarding' ),
						'value' => $parcel->reference,
					),
					array(
						'name'  => __( 'Tracking number', 'colisly-parcel-forwarding' ),
						'value' => $parcel->tracking_number,
					),
					array(
						'name'  => __( 'Weight (kg)', 'colisly-parcel-forwarding' ),
						'value' => $parcel->weight,
					),
					array(
						'name'  => __( 'Status', 'colisly-parcel-forwarding' ),
						'value' => COLISLY_Parcels::status_label( $parcel->status ),
					),
					array(
						'name'  => __( 'Received on', 'colisly-parcel-forwarding' ),
		...
		foreach ( COLISLY_Shipments::for_client( (int) $client->id ) as $shipment ) {
			$items[] = array(
				...
				'group_label' => __( 'Shipments (Colisly Parcel Forwarding)', 'colisly-parcel-forwarding' ),
				'item_id'     => 'colisly-shipment-' . (int) $shipment->id,
				'data'        => array(
					array(
						'name'  => __( 'Reference', 'colisly-parcel-forwarding' ),
						'value' => $shipment->reference,
					),
					array(
						'name'  => __( 'Carrier', 'colisly-parcel-forwarding' ),
					...
					array(
						'name'  => __( 'Requested on', 'colisly-parcel-forwarding' ),
						'value' => $shipment->requested_at,
					),
					array(
						'name'  => __( 'Total', 'colisly-parcel-forwarding' ),
		...
		foreach ( COLISLY_Documents::for_client( (int) $client->id, true ) as $document ) {
			$items[] = array(
				...
				'group_label' => __( 'Documents (Colisly Parcel Forwarding)', 'colisly-parcel-forwarding' ),
				'item_id'     => 'colisly-document-' . (int) $document->id,
				'data'        => array(
					array(
						'name'  => __( 'Title', 'colisly-parcel-forwarding' ),
						'value' => $document->title,
					),
					array(
						'name'  => __( 'Added on', 'colisly-parcel-forwarding' ),
	...
	public static function erase( $email ) {
		...
		COLISLY_History::log( $client_id, 'privacy_erasure', __( 'Personal data erased following a privacy request.', 'colisly-parcel-forwarding' ) );

		return array(
			'items_removed'  => $removed,
			'items_retained' => true,
			'messages'       => array(
				__( 'Parcel and shipment records are retained as business/accounting records; documents, phone number, notes, tracking numbers and photos have been removed.', 'colisly-parcel-forwarding' ),
			),
			'done'           => true,
		);
	}
